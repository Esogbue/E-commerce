...
export default {
    ...
    mounted() {
        document.title = 'Checkout | Mstores'

        this.cart = this.$store.state.cart

        if (this.cartTotalLength > 0) {
            this.paystack = Paystack('pk_test_b372d755f3e7c55ca57c091433e593e2013321d4')
            const elements = this.paystack.elements();
    ...
    methods: {
        ...
        submitForm() {
            ...
            if (!this.errors.length) {
                ...
                this.paystack.createToken(this.card).then(result => {                    
                    if (result.error) {
                        this.$store.commit('setIsLoading', false)

                        this.errors.push('Something went wrong with Paystack. Please try again')
